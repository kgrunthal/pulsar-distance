    ...
    for ii, ltp in enumerate(psrs):
        ...
        for s in sgn:
            if s=='RN':
                ...
                LT.add_gwb(ltp, gwAmp=5e-14, alpha=-13./3.)
                outstr += ' added GWB'
...
def PTA_model(psrs, obtimes, ptamodel, mode=None):
    ...
    for m in model:
        if m =='TM':
            ...
            #s.append(blocks.common_red_noise_block(psd='powerlaw', prior='log-uniform', gamma_val = 0., Tspan = Tspan, name='gw', modes=np.array([mode])))
...
def do_analysis(PSRs, obstimes, signal, ptamodel, Ncgw = 1, fgw=[5e-8], mc=[10**9], zeta=1., outdir = '', cornerplot=False, Filter=True, psrTerm = True):
    ...
    produce_output('{}/'.format(outdir), cornerplot=cornerplot)
...
def main():
    ...
    ePSRs, pta, setpars = do_analysis(PSRs, obstimes, args.signal, args.ptamodel, fgw=np.array(args.fgw)*1e-9, mc=10**np.array(args.lmc), zeta=args.zeta, outdir=args.outdir, Ncgw=args.ncgw,  cornerplot=args.cornerplot, psrTerm=args.psrTerm, Filter=False)
    ...
    np.savetxt(args.result, out, delimiter='\t')
