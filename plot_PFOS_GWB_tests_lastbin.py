# -*- coding: utf-8 -*-
"""
Created on Thu May  2 11:39:38 2024
...
def main():
    ...
    GWBpl_powerlaw_NM = np.loadtxt(basefolder + 'OS_powerlaw_GWB_lastbin_{}_NM.txt'.format(run_pl)).transpose()
    GWBbroken_powerlaw_NM = np.loadtxt(basefolder + 'OS_powerlaw_GWBbroken_lastbin_{}_NM.txt'.format(run_bpl)).transpose()
    GWBsinglebin_powerlaw_NM = np.loadtxt(basefolder + 'OS_powerlaw_GWBsinglebin_lastbin_{}_NM.txt'.format(run_sgl)).transpose()
        
    GWBpl_fullspectrum_NM = np.loadtxt(basefolder + 'OS_fullspectrum_GWB_lastbin_{}_NM.txt'.format(run_pl)).transpose()
    GWBbroken_fullspectrum_NM = np.loadtxt(basefolder + 'OS_fullspectrum_GWBbroken_lastbin_{}_NM.txt'.format(run_bpl)).transpose()
    GWBsinglebin_fullspectrum_NM = np.loadtxt(basefolder + 'OS_fullspectrum_GWBsinglebin_lastbin_{}_NM.txt'.format(run_sgl)).transpose()
    
    
    ###########################################################################
    ###  noisemarginalised frequency OS values  ###############################
    
    GWBpl_spectrum_NM = np.loadtxt(basefolder + 'OS_spectrum_GWB_lastbin_{}_NM.txt'.format(run_pl))
    GWBbroken_spectrum_NM = np.loadtxt(basefolder + 'OS_spectrum_GWBbroken_lastbin_{}_NM.txt'.format(run_bpl))
    GWBsinglebin_spectrum_NM = np.loadtxt(basefolder + 'OS_spectrum_GWBsinglebin_lastbin_{}_NM.txt'.format(run_sgl))
    
    ###########################################################################
    ###  maxlike values  ######################################################
    maxlike_freespectrum_GWBpl = json.load(open('./MCMCout_GWB_lastbin_{}/freespectrum/maxlike.json'.format(run_pl)))
    maxlike_freespectrum_GWBbroken = json.load(open('./MCMCout_GWBbroken_lastbin_{}/freespectrum/maxlike.json'.format(run_bpl)))
    maxlike_freespectrum_GWBsinglebin = json.load(open('./MCMCout_GWBsinglebin_lastbin_{}/freespectrum/maxlike.json'.format(run_sgl)))
    
    maxlike_powerlaw_GWBpl = json.load(open('./MCMCout_GWB_lastbin_{}/powerlaw/maxlike.json'.format(run_pl)))
    maxlike_powerlaw_GWBbroken = json.load(open('./MCMCout_GWBbroken_lastbin_{}/powerlaw/maxlike.json'.format(run_bpl)))
    maxlike_powerlaw_GWBsinglebin = json.load(open('./MCMCout_GWBsinglebin_lastbin_{}/powerlaw/maxlike.json'.format(run_sgl)))
    
    ###########################################################################
    ###########################################################################
    outdir = './out/powerlaw_vs_spectrum/last_bin/'
