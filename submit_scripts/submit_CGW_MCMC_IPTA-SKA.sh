...
#for pd in over1.0 over1.5 over2.0; do
for pd in full far ipta; do
    ...
    if [ ! -f "$RESULT_DIR_noPT/maxlike.json" ]; then
        # IPTA only
        #sbatch -p short.q --time=04:00:00 --mem=20GB --output=$MCMC_OUTDIR/slurm_output/CGW_noPT.out --error=$MCMC_OUTDIR/slurm_output/CGW_noPT.err --job-name=$NAME --wrap="singularity exec -B /scratch/kgrunthal/,/hercules/results/kgrunthal/,/u/kgrunthal/ /scratch/kgrunthal/MPTA_singularity/ python3 CGW_search_IPTA-SKA.py --basedir $MCMC_OUTDIR --outdir $RESULT_DIR_noPT --ptamodel TM,WN,CGW --noisefile /u/kgrunthal/HD/ipta_sim/WN_dictionary.json --analysis --use_distance"

        # SKA
        sbatch -p long.q --time=08:00:00 --mem=20GB --output=$MCMC_OUTDIR/slurm_output/CGW_noPT.out --error=$MCMC_OUTDIR/slurm_output/CGW_noPT.err --job-name=$NAME --wrap="singularity exec -B /scratch/kgrunthal/,/hercules/results/kgrunthal/,/u/kgrunthal/ /scratch/kgrunthal/MPTA_singularity/ python3 CGW_search_IPTA-SKA.py --basedir $MCMC_OUTDIR --outdir $RESULT_DIR_noPT --ptamodel TM,WN,CGW --noisefile /u/kgrunthal/HD/ska_sim/FULL_WN_dictionary.json --analysis --use_distance"
    fi
