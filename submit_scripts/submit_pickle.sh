...
for lmc in 8.7; do    
    for pd in 1.0 4.0; do
        ...
        #MCMC_OUTDIR=/u/kgrunthal/HD/MCMCout_isotropic_scaleto$lmc\_CGW9.0_pd$pd\_$i/
        MCMC_OUTDIR=/u/kgrunthal/HD/MCMCout_galactic20_5h_CGW$lmc\__pd$pd\_$i/
        ...
        # scale to 9.7
        #sbatch -p short.q --time=00:15:00 --mem=8GB --output=$MCMC_OUTDIR/slurm_output/pickle.out --error=$MCMC_OUTDIR/slurm_output/pickle.err --job-name=$NAME --wrap="singularity exec -B /scratch/kgrunthal/,/hercules/results/kgrunthal/,/u/kgrunthal/ /scratch/kgrunthal/EPTA_singularity/ python3 /u/kgrunthal/HD/make_pickle.py --par $PAR_DIR --outdir $MCMC_OUTDIR --result $RESULT_DIR/$OUTFILE --fgw 22.3 --ncgw 1 --signal CGW --lmc $lmc --zeta -1 --pdistance $pd --pdist_fix --psrTerm --scale_to 8.7"
