...
for lmc in 9.0 ; do
    for pd in 1.0 4.0 ; do
        ...
        RESULT_DIR=/u/kgrunthal/HD/out/isotropic_scaleto8.7/
        
        #MCMC_OUTDIR=/u/kgrunthal/HD/recovery_simulations/ipta/dl15/MCMCout_IPTA_WN_CGW_RA12hDEC0deg_$lmc\_$pd\_$i/
        MCMC_OUTDIR=/u/kgrunthal/HD/MCMCout_isotropic_scaleto8.7_CGW$lmc\_earth/
        ...
        sbatch -p short.q --time=00:30:00 --mem=12GB --output=$MCMC_OUTDIR/slurm_output/analysis.out --error=$MCMC_OUTDIR/slurm_output/analysis.err --job-name=$NAME --wrap="singularity exec -B /scratch/kgrunthal/,/hercules/results/kgrunthal/,/u/kgrunthal/ /scratch/kgrunthal/MPTA_singularity/ python3 /u/kgrunthal/HD/HD_with_OS_analysis.py --par $PAR_DIR --outdir $MCMC_OUTDIR/PFOS/ --result $RESULT_DIR/run_$i\/$OUTFILE --psrpickle $MCMC_OUTDIR/psrs.pkl --ptamodel TM,WN,CRN_fs --OStype spectrum --N 1000"
