...
#basedir='/u/kgrunthal/HD/epta_sim/test_bin7_A5e-12/'
basedir='/u/kgrunthal/HD/epta_sim/test_GW7_RN12/'
...
spec[bin_pos_1*howml] = 5e-12*np.ones(1)
#spec[bin_pos_2*howml] = 5e-12*np.ones(1)
...
spec_nc = 1e-50*np.ones(len(freq))
spec_nc[bin_pos_2*howml] = 1e-12*np.ones(1)
userSpec_nc = np.asarray([freq, spec_nc]).T

createGWB(psrs, Amp=Amp, gam=gamma, howml=howml, userSpec=userSpec_nc, noCorr=True)
...
plt.axvline(fs[bin_pos_1-1], color='gray', ls=':')
plt.axvline(fs[bin_pos_2-1], color='gray', ls=':')
...
axs[0].axvline(fs[bin_pos_1-1], color='gray', ls=':')
axs[0].axvline(fs[bin_pos_2-1], color='gray', ls=':')
